...
SDLLoadGameCode(char* SourceDynLibName)
{
  ...
  if(Result.GameCodeDynLib)
	...
	dlsym(Result.GameCodeDynLib, "GameUpdateAndRender");
      Result.GetSoundSamples = (game_get_sound_samples *)
	dlsym(Result.GameCodeDynLib, "GameGetSoundSamples");
...
{
  if(dlclose(GameCode->GameCodeDynLib))
    {
      printf("Game code not unloaded properly\n");
      char* Err = dlerror();
      printf("Dynamic linking error:\n%s\n", Err);
    }
  GameCode->GameCodeDynLib = 0;
